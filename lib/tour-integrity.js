...
// Script tags index.js depends on. Order matters: vendors + data.js before index.js.
...
// Stylesheets linked from generated index.html.
const REQUIRED_STYLESHEETS = ['vendor/reset.min.css', 'style.css'];

// Scene ids are interpolated into CSS attribute selectors:
//   querySelector('#sceneList .scene[data-id="' + scene.data.id + '"]')
// Quotes / whitespace / brackets break that selector and throw on .addEventListener.
const SAFE_SCENE_ID_PATTERN = /^[A-Za-z0-9_-]+$/;
...
function extractScriptSrcs(html) {
  return [...html.matchAll(/<script\b([^>]*)>/gi)]
    .map((match) => {
      const srcMatch = match[1].match(/\bsrc=(["'])([^"']*)\1/i);
...
function extractStylesheetHrefs(html) {
  return [...html.matchAll(/<link\b([^>]*)>/gi)]
    .map((match) => match[1])
    .filter((attrs) => /\brel=(["'])stylesheet\1/i.test(attrs))
    .map((attrs) => {
...
/**
 * index.js assumes vendor + APP_DATA globals exist before it runs.
 * Wrong order (or a dropped script tag) hard-fails boot even when files exist on disk.
 */
function bootResourceErrors(html) {
  const errors = [];
  const scripts = extractScriptSrcs(html);
  const stylesheets = extractStylesheetHrefs(html);

  for (const href of REQUIRED_STYLESHEETS) {
    if (!stylesheets.includes(href)) {
      errors.push(`index.html is missing stylesheet ${href}`);
    }
  }

  for (const src of REQUIRED_BOOT_SCRIPTS) {
    if (!scripts.includes(src)) {
      errors.push(`index.html is missing boot script ${src}`);
    }
  }

  const dataIndex = scripts.indexOf('data.js');
  const playerIndex = scripts.indexOf('index.js');
  const marzipanoIndex = scripts.indexOf('vendor/marzipano.js');
  if (dataIndex !== -1 && playerIndex !== -1 && dataIndex > playerIndex) {
    errors.push('index.html must load data.js before index.js');
  }
  if (marzipanoIndex !== -1 && playerIndex !== -1 && marzipanoIndex > playerIndex) {
    errors.push('index.html must load vendor/marzipano.js before index.js');
  }
  return errors;
}

function hasTitleBarSceneName(html) {
  // updateSceneName does: sceneNameElement.innerHTML = ... via #titleBar .sceneName
  return (
    /\bid=["']titleBar["']/.test(html) &&
    /\bclass=(["'])[^"']*\bsceneName\b[^"']*\1/.test(html)
  );
}

function isCssAttributeSafeSceneId(id) {
  return typeof id === 'string' && SAFE_SCENE_ID_PATTERN.test(id);
}
...
function settingsChromeMismatches(settings, html) {
  ...
  if (settings.viewControlButtons) {
    ...
    // CSS shows the on-screen view controls only when body has this class.
    if (!/<body\b[^>]*\bclass=(["'])[^"']*\bview-control-buttons\b[^"']*\1/i.test(html)) {
      mismatches.push(
        'settings.viewControlButtons is true but index.html body lacks class view-control-buttons'
      );
    }
  }
  return mismatches;
...
function validateTourPackage(tourDir) {
  ...
  for (const scene of scenes) {
    ...
    } else if (!isCssAttributeSafeSceneId(scene.id)) {
      errors.push(
        `scene.id "${scene.id}" is unsafe for CSS attribute selectors used by index.js`
      );
    }
  ...
  if (!hasTitleBarSceneName(html)) {
    errors.push('index.html is missing #titleBar .sceneName required by updateSceneName');
  }
  for (const bootError of bootResourceErrors(html)) {
    errors.push(bootError);
  }
  for (const mismatch of settingsChromeMismatches(appData.settings, html)) {
    errors.push(mismatch);
  }
  if (!/<body\b[^>]*\bclass=(["'])[^"']*\bmultiple-scenes\b[^"']*\1/i.test(html)) {
    errors.push('index.html body lacks class multiple-scenes required for multi-scene layout');
  }
